...
|------|--------------|-------------|
| **In-memory** | `cs.AIToolMemory.new()` | Entries stored in a collection on the class instance. Lost when the process ends. |
| **Database** | `cs.AIToolMemory.new({dataclass: "Memory"})` | Entries persisted to a 4D dataclass via ORDA. Survives restarts. |
